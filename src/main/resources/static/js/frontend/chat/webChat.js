/**
 聊天室操作* 
 */

$(document).ready(function() {
  // 預設聊天室是收起狀態，隱藏chat_container
  $('.chat_container').hide();

  // 點擊左下縮圖按鈕，切換聊天室展開與收起
  $('.chat_button').on('click', function() {
    $('.chat_container').toggle(); // 切換顯示或隱藏
    $('.chat_button').toggleClass('active'); // 按鈕樣式切換(可自定義CSS)
  });
});


 let stompClient = null;
 let cutrrentRoomId = null;
 
 function connect(roomId){
	cutrrentRoomId = roomId;
	const socket = new SockJS("/ws");
	stompClient = Stomp.over(socket);
	
	stompClient.connect({}, function(){
		stompClient.subscribe(`/topic/${roomId}`, function(message){
			const chat = JSON.parse(message.body);
			appendMessage(chat, chat.sender === "me" ? "you-messsage" : "other-message");
			
		});
		loadHistory(roomId);
	})
 }
 
 function sendMessage(){
	const msg = $(".sendMsg").val();
	if(msg.trim() === ""){
		return;
	}
	
	const chat = {
		roomId: cutrrentRoomId,
		sender: "me",
		message : msg,
		timestamp : new Date().toLocaleString("zh-TW", {hour12:false})
	};
	
	stompClient.send(`/app/chat/${cutrrentRoomId}`, {}, JSON.stringify(chat));
	$(".sendMsg").val("");
 }
 
 function loadHistory(roomId){
	$("#chatRoom").html("");
	$.get(`/chat/history/${roomId}`, function(messages){
		messages.forEach(chat => {
			const type = chat.sender === "me" ? "you-message" : "other-message";
			appendMessage(chat, type);
		});
	});
 }
 
 function appendMessage(chat, type){
	const msgHtml = `
	<div class="message_row ${type}>
		<div class="message-content">
			${type === "other-message" ? `<img class="head" src="" alt="">` : ""}
			<div class="message-text">${chat.message}</div>
			<div class="message-time">${chat.timestamp}</div>
		</div>
	</div>`;	
	
	$("#chatRoom").append(msgHtml);
	$("#chatRoom").scrollTop($("#chatRoom")[0].scrollHeight);
	
 }
 
 $(document).ready(function(){
	$(".chatListTag").on("click", function(){
		$(".chatListTag").removeClass("active");
		$(this).addClass("active");
		const roomId = $(this).data("room-id");
		connect(roomId);
	});
	
	$(".send_icon").on("click", function(){
		sendMessage();
	});
	
	$(".sendMsg").on("keypress", function(e){
		if(e.which === 13){
			sendMessage();
		}
	});
 });
 
 
 
 
 const web_Me = {
    "name": "Johnny",
    "title":"網頁前端",
    "email":"nintendof1@gmail.com",
    "photo":"img/me/Johnny.png",
    "photo2":"img/me/Johnny2.jpg"
  }
  
  const web_Friends = [
    {
      "name": "Alpha_Team",
      "title":"Alpha 測試團隊",
      "email":"Alpha_Team@gmail.com",
      "photo":"img/friends/Alpha_Team.png",
      "messageNum":3
    },
    {
      "name": "Beta_Team",
      "title":"Beta 測試團隊",
      "email":"Beta_Team@gmail.com",
      "photo":"img/friends/Beta_Team.png",
      "messageNum":2
    },
    {
      "name": "Big Boss",
      "title":"公司老大",
      "email":"BigBoss@gmail.com",
      "photo":"img/friends/BigBoss.png",
      "messageNum":0
    },
    {
      "name": "David",
      "title":"專案經理",
      "email":"David@gmail.com",
      "photo":"img/friends/David.png",
      "messageNum":7
    },
    {
      "name": "Final_Check",
      "title":"最後檢測",
      "email":"Final_Check@gmail.com",
      "photo":"img/friends/Final_Check.png",
      "messageNum":0
    },
    {
      "name": "Grace",
      "title":"芮絲",
      "email":"Grace@gmail.com",
      "photo":"img/friends/Grace.png",
      "messageNum":0
    },
    {
      "name": "Judy",
      "title":"美編美工",
      "email":"Judy@gmail.com",
      "photo":"img/friends/Judy.png",
      "messageNum":0
    },
    {
      "name": "LittleBoss",
      "title":"二老闆",
      "email":"LittleBoss@gmail.com",
      "photo":"img/friends/LittleBoss.jpg",
      "messageNum":0
    },
    {
      "name": "Peter",
      "title":"楊彼得",
      "email":"Peter@gmail.com",
      "photo":"img/friends/Peter.png",
      "messageNum":6
    },
    {
      "name": "Steve",
      "title":"網頁後端",
      "email":"Steve@gmail.com",
      "photo":"img/friends/Steve.png",
      "messageNum":14
    },
    {
      "name": "Susan",
      "title":"服務總機",
      "email":"Susan@gmail.com",
      "photo":"img/friends/Susan.png",
      "messageNum":8
    },
    {
      "name": "Tech_boy",
      "title":"技術客服",
      "email":"Tech_boy@gmail.com",
      "photo":"img/friends/Tech_boy.png",
      "messageNum":0
    }  ,
    {
      "name": "Tony",
      "title":"技術客服",
      "email":"Tony@gmail.com",
      "photo":"img/friends/Tony.png",
      "messageNum":1
    }
  ]
 
 
 
 let friendCard = '';
  
  web_Friends.map(data => {
    if(data.messageNum > 0){
      if(data.name == 'David'){
        friendCard += `
          <div class="chatListTag active">
            <div class="head"><img src="${data.photo}" alt=""></div>
            <div class="mytext">
              <div class="name">${data.name}</div>
              <div class="dec">${data.title}</div>
            </div>
            <div class="msg_num">${data.messageNum}</div>
          </div>
        `
      } else {
        friendCard += `
          <div class="chatListTag">
            <div class="head"><img src="${data.photo}" alt=""></div>
            <div class="mytext">
              <div class="name">${data.name}</div>
              <div class="dec">${data.title}</div>
            </div>
            <div class="msg_num">${data.messageNum}</div>
          </div>
        `
      }
    } else {
      friendCard += `
        <div class="chatListTag">
          <div class="head"><img src="${data.photo}" alt=""></div>
          <div class="mytext">
            <div class="name">${data.name}</div>
            <div class="dec">${data.title}</div>
          </div>
        </div>
      `
    }
  })
  document.getElementById('myChatList').innerHTML = friendCard;